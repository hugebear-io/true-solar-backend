...
type ElasticSearchRepoPort interface {
	// createIndexIfNotExist(index string) error
	BulkIndex(docs []interface{}) error
	UpsertSiteStation(docs []SiteItem) error
	DeleteSiteStationDocument() error
	SearchIndex() *elastic.SearchService
	QueryPerformanceLow(duration int, EfficiencyFactor float64, FocusHour int, thresholdPercentage float64) ([]*elastic.AggregationBucketCompositeItem, error)
	QuerySumPerformanceLow(duration int) ([]*elastic.AggregationBucketCompositeItem, error)
	QueryPerformanceOK(duration int, EfficiencyFactor float64, FocusHour int, thresholdPercentage float64) ([]*elastic.AggregationBucketCompositeItem, error)
}
